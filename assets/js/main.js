	...
	var validateContactForm = function() {
		$('#contact-form-submit').on('click', function(e) {
			if($('#contact-form-name').val().length != 0 &&
					...
					$('#contact-form-captcha').val().length != 0 &&
					$('#contact-form-captcha').val() != "26"
				) {
					e.preventDefault();
					$('#contact-form-captcha').css('background-color', '#FFCCD3');
					$.notify({message: "Das eingegebene Captcha ist nicht korrekt."}, {type: "danger"});
			}
			else {
				$('#contact-form').submit();
			}
